...
\section{Chapter 5}
$\xi = -\dfrac{dV}{dx}$ \\
$V_{bi} = \dfrac{kT}{q}ln(\dfrac{N_AN_D}{n_i^2})$
\subsection{Poisson's Equation}
\begin{equation}
	\dfrac{d\xi}{dx} \cong
	\begin{cases}
	\dfrac{q}{K_s\varepsilon_0}(N_D - N_A)\longrightarrow -x_p\leq x\leq x_n \\
	0 \longrightarrow x\leq -x_p \cap x\geq x_n
	\end{cases}
\end{equation}
...
Diamond Lattice  & 8 atoms per unit cell\\
\end{tabular}
\subsection{Miller Indices}
Shift axes around to find intercepts, divide by common denominator, take the inverse to find (h k l). [h k l] is just the normal vector. Must be integer values.

\section{Bandgap}
Insulators have a bandgap of 5-7 eV, Semiconductors have a bandgap of ~1 eV, Conductors have $E_v > E_c$. \\
